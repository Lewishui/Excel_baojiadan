...
namespace QCAuto
{
    public partial class shimingfang_2008 : Form
    {
        ...
        public shimingfang_2008(string password)
        {
            try
            {
                //bat_dsoframer();
                ...
                if (ssd == 0)
                {
                    ...
                    if (!ZFCEPath.Contains("D:\\Devlop\\报价单\\ewm\\Excel_baojiadan\\QCAuto\\bin\\Debug"))
                    {
                       // bool status = SharedTool.connectState(ipadd, @netuser, netpassword);
                        ...
                        {
                            MessageBox.Show("没有找到此路径或此文件，请保证共享文件存在!" + ZFCEPath);
                        ...
                        if (islocalpath == false)
                            ...
                            if (File.Exists(ZFCEPath2))
                                status = true;
                            else
                                status = false;
                        if (!File.Exists(ZFCEPath2) && status != true)
                        {
                            MessageBox.Show("没有找到此路径或此文件，请保证共享文件存在!" + ZFCEPath2);
                            System.Environment.Exit(0);
                            return;
                        }
                    }
        ...
        private void Local_IP()
        {
            ...
            if (fileText.Length > 0 && fileText[0] != null && fileText[0] != "")
            {
                ...
                if (fileText[4] != null && fileText[4] != "")
                    ZFCEPath2 = fileText[4];
                if (!ZFCEPath2.Contains("\\"))
                {
                    ZFCEPath2 = "" + AppDomain.CurrentDomain.BaseDirectory + "shimingfang_2008\\" + ZFCEPath2;
                    islocalpath = true;
